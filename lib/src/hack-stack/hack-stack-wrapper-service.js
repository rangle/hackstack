  ...
  .factory('hackWrap', function ($q, $http, hackStackUtils, API_BASE) {
    function createWrapper(endpoint, mockData, options) {
      ...
      if (options) {
        ...
        /**
         * Possible values for options.priority are 'server' or 'mock'.
         */
        if(options.priority) {
          options.priority = options.priority || 'server';
        }
      }
       ...
       * @returns {*}
       */
      function deepMerge(priorityObj, mergingObj) {
        var priObj = angular.copy(priorityObj);  //Make copies so we don't hold
        var newObj = angular.copy(mergingObj);   //references.

        for (prop in priObj) {
          if(priObj[prop] && typeof priObj[prop] === 'object') {
            newObj[prop] = merge(priObj[prop], newObj[prop]);
          } else {
        ...
        return $http.get(baseEndpoint)
          .then(function (response) {
            var newData = R.map(function (item) {
              if(options.priority && options.priority === 'mock') {
                return deepMerge(mockData, item);
              } else {
                return deepMerge(item, mockData);
              }
      ...
      function processGet(id) {
        return $http.get([baseEndpoint, id].join('/'))
          .then(function (response) {
            if(options.priority && options.priority === 'mock') {
              response.data = deepMerge(mockData, response.data);
            } else {
              response.data = deepMerge(response.data, mockData);
            }
            return response;
          });
      }
