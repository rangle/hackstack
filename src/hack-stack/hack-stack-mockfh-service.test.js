...
describe('Hack Stack FeedHenry tests', function () {
  ...
  beforeEach(inject(function (_mockFH_) {
    ...
    fh.addMockEndpoint(path, [mockObject1, mockObject2]);
    $window.$fh = {
      'cloud': cloudSpy
    };
  }));
  ...
  describe('sanity checks', function () {
    ...
    it('should use options.makeCompareFn if provided', function () {
      path = generateNewPath();
      var options = angular.copy(defaultOptions);
      options.path = path;
      options.data = mockObject2.title;
      fh.addMockEndpoint(path, [mockObject1, mockObject2], {
        'makeCompareFn': function (requestData) {
          return function (targetData) {
            return targetData.title === requestData;
          };
        }
      });
      fh.cloud(options, function (res) {
        expect(res.data).to.deep.equal(mockObject2);
      });
      $timeout.flush();
    });
  });
