...
describe('Hack Stack Wrapper tests', function () {
  ...
  beforeEach(inject(function (_hackstack_, _$timeout_, _$q_, _$httpBackend_,
    ...
    $httpBackend.whenGET([endpoint].join('/'))
      .respond([incompleteObject, incompleteObject]);
    $httpBackend.whenGET(new RegExp([endpoint, '.*'].join('/')))
  ...
  describe('GET tests', function () {
    ...
    it(
      ...
      function () {
        ...
        var wrapper = hw(endpoint, mockData1);
        wrapper.disableRandomErrors(true);
        $httpBackend.expectGET([endpoint, id].join('/'));
    ...
    it(
      ...
      function () {
        var wrapper = hw(endpoint, mockData1);
        wrapper.disableRandomErrors(true);
        $httpBackend.expectGET([endpoint].join('/'));
        var response = wrapper.getAll();
        $timeout.flush();
        $httpBackend.flush();
      });
  ...
  describe('CREATE tests', function () {
    it('should forward create requests', function () {
      ...
      var wrapper = hw(endpoint, mockData1);
      wrapper.disableRandomErrors(true);
      $httpBackend.expectPOST([endpoint].join('/'))
        .respond(200, 'ok', {
          'location': [endpoint, id].join('/')
        });
  ...
  describe('Update tests', function () {
    it('should issue an update request to origin', function () {
      ...
      var wrapper = hw(endpoint, mockData1);
      wrapper.disableRandomErrors(true);
      $httpBackend.expectPOST([endpoint, id].join('/'))
        .respond(200, 'ok');
